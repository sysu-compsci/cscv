...
public:
    ...
    void collect_data_arr_and_memset_0(int key, int count, Element_type* data);
    ...
    std::vector<int> get_effective_keys() const;

    const Result_column& get_column(int key) const { return m_cols.at(key); }

    bool check_if_key_exist(int key) const {
        if (m_cols.count(key) != 0 && m_cols.at(key).m_results.size() != 0)
            return true;
        return false;
    }
};
...
template <class Element_type>
void Result_table<Element_type>::collect_data_arr_and_memset_0(int key, int count, Element_type* data) {
    ...
    memset(data, 0, sizeof(Element_type) * count);
}
...
std::string Result_table<Element_type>::get_rela_to_std_string_for_col(int col_id, String_table& str_table) const {
    ...
    Element_type abs_diff_sum = 0;
    Element_type abs_diff_max = 0;
    Element_type max, min;
    Element_type sum = 0, std_sum = 0;
    ...
    for (int i = 0; i < std_col.m_results.size(); i++) {
        ...
        std_sum += std_col.m_results[i];
        abs_diff_sum += diff;
        abs_diff_max = std::max(abs_diff_max, diff);
        if (i == 0) {
    ...
    str_table.set_value(current_col.m_label, "sum_diff", std::abs(sum - std_sum));
    str_table.set_value(current_col.m_label, "min", min);
    str_table.set_value(current_col.m_label, "max", max);
    str_table.set_value(current_col.m_label, "abs_diff_sum", abs_diff_sum);
    str_table.set_value(current_col.m_label, "abs_diff_max", abs_diff_max);
...
template <class Element_type>
std::vector<int> Result_table<Element_type>::get_effective_keys() const {
    std::vector<int> ret;
    ...
    for (auto p : m_cols) {
        ...
        if (0 != current_col.m_results.size() && p.first != 0) {
