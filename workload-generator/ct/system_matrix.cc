    ...
    void init(const Block_shape *range_img_surrounded, int img_size) {
        ...
        for (i = 0; i <= 1; i++) {
            ...
            if (range_img_surrounded->m_idx_end[i] > img_size - 1)
                tri_idx_end[i] = range_img_surrounded->m_dim_block[i] - 3;  // TODO(Entityless): Check.
            else
                tri_idx_end[i] = range_img_surrounded->m_dim_block[i] - 2;  // TODO(Entityless): Check.
        }
...
void generate_sub_system_matrix_constant_inner(Img_param img_param, Range_nd<2> img_range, Range_1d angle_range,
    ...
    static __thread Seq_map<Img_param, System_matrix_linear_buffer*> g_sml_gen_buffer;

    // get buffers
    System_matrix_linear_buffer* buffers;
    if (g_sml_gen_buffer.count(img_param) == 0) {
        buffers = new System_matrix_linear_buffer(img_param);
    ...
    local_IdxImgStart[DIM_ROW] = img_range[0].m_start;
    local_IdxImgStart[DIM_COL] = img_range[1].m_start;
    ...
    for (int angle_id = angle_range.m_start; angle_id < angle_range.m_start + angle_range.m_size; angle_id++) {
        ...
        for (int y = 0; y < img_range[0].m_size; y++) {
            for (int x = 0; x < img_range[1].m_size; x++) {
                int nIndex2D[2] = {y, x};
                int tmpColIdx = x + y * img_range[1].m_size;
