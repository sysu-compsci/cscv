...
void CSR_matrix<Element_type>::multiply_dense_vector(const Dense_vector<Element_type>& in_vec, const Dense_vector<Element_type>& out_vec) {
    ASSERT_AND_PRINTF(in_vec.get_size() >= m_num_col && out_vec.get_size() >= m_num_row, "mtx x = %d, in_vec sz = %d, mtx y = %d, out_vec sz = %d\n",
...
template <class Element_type>
void COO_matrix_buffer<Element_type>::write_to_txt(std::string filename) {
    std::ofstream ofs;
    ofs.open(filename);

    ofs << m_num_row << " " << m_num_col << " " << m_nz_count << std::endl;
    
    for (int i = 0; i < m_nz_count; i++) {
        ofs << m_nz_row_idx[i] << " " << m_nz_col_idx[i] << " " << m_vals[i] << std::endl;
    }
}
